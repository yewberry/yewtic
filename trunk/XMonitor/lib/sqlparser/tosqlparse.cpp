...
const toSQLParse::statement &toSQLParse::statement::operator =(
...
bool toSQLParse::statement::operator ==(const statement &stat) const {
	if (Type != stat.Type || Comment != stat.Comment || String != stat.String)
		return false;
	if (SubTokens) {
		...
		if ((*SubTokens) != (*stat.SubTokens))
			return false;
	} else if (stat.SubTokens && stat.SubTokens->begin()
			!= stat.SubTokens->end())
		return false;
	return true;
}
...
QString toSQLParse::stringTokenizer::getToken(bool forward, bool comments) {
	...
	enum {
		space, any, identifier, string, comment, label, bindOpen, bindClose
	} state = space;
	...
	while ((forward && Offset < int(String.length())) || (!forward && Offset
			>= 1)) {
		if (!forward)
			Offset--;
		c = String[Offset];
		if (c == '\n')
			Line++;
		if ((forward && Offset < int(String.length() - 1)) || (!forward
				&& Offset > 0))
		...
		if (state == space) {
			if (forward && c == '-' && nc == '-') {
				int spos = Offset;
				if (forward)
					for (Offset++; Offset < int(String.length())
							&& String[Offset] != '\n'; Offset++)
						;
				if (comments)
			...
			if (c == '/' && nc == '*')
				state = comment;
			else if ((forward && c == '<' && nc == '<') || (!forward && c
					== '>' && nc == '>'))
				state = label;
			else if (!c.isSpace())
				state = any;
			...
			case any:
				...
				} else if (c == '\'' || c == QUOTE_CHARACTER) // SQLITEMAN c == analyzer().quoteCharacter())
				{
					...
					for (int i = 0; Operators[i]; i++) {
						if ((forward && c == Operators[i][0] && nc
								== Operators[i][1]) || (!forward && nc
								== Operators[i][0] && c == Operators[i][1])) {
							if (forward)
								token += nc;
							else
								token.prepend(nc);
...
toSQLParse::statement toSQLParse::parseStatement(tokenizer &tokens,
	...
	bool nokey = false;
	bool block = false;
	for (QString token = tokens.getToken(true, true); !token.isNull(); token
			= tokens.getToken(true, true)) {
		...
		if (upp == "BEGIN" && (first.isNull() || first == "BEGIN")) {
			//             qDebug() << "plain BEGIN";
			ret.subTokens().insert(ret.subTokens().end(), statement(
					statement::Keyword, token, tokens.line()));
			nokey = false;
		} else if (first != ("END") && ((first == ("IF") && upp == ("THEN"))
				|| upp == ("LOOP") || upp == ("DO")
				|| (/*syntax.declareBlock()*/true && upp == ("DECLARE"))
				|| (block && upp == ("AS")) || (block && upp == ("IS"))
			...
			do {
				...
				if (cur.subTokens().begin() != cur.subTokens().end()
						&& (*(cur.subTokens().begin())).String.toUpper()
								== ("BEGIN"))
					dcl = false;
			} while (cur.subTokens().begin() != cur.subTokens().end()
					&& (*cur.subTokens().begin()).String.toUpper() != ("END"));
			return blk;
		} else if (((first == "IF" && upp == "THEN") || (first == "WHEN" && upp
				== "THEN") || (first == "ELSIF" && upp == "THEN") || upp
				== ("BEGIN") || upp == ("EXCEPTION") || first == ("ELSE"))
				&& !lst) {
			//              qDebug() << "else if first==IF";
			ret.subTokens().insert(ret.subTokens().end(), statement(
					statement::Keyword, token, tokens.line()));
			return ret;
		} else if (first == ("ASSIGN") || first == ("SET") || first
				== ("PROMPT") || first == ("COLUMN") || first == ("SPOOL")
				|| first == ("STORE") || first == ("REMARK") || first
				== ("REM")) {
			//              qDebug() << "ASSIGN";
			ret.subTokens().insert(ret.subTokens().end(), statement(
					statement::Keyword, token, tokens.line()));
			...
			for (QString tmp = tokens.getToken(true, true); line
					== tokens.line(); tmp = tokens.getToken(true, true))
				ret.subTokens().insert(ret.subTokens().end(), statement(
						statement::Token, tmp, line));
			tokens.setLine(line);
		...
		// 		if (upp == (",") ||
				...
				(isKeyword(upp) && upp != ("NOT") && upp != ("IS") && upp
						!= ("LIKE") && upp != ("IN") && upp != ("ELSE") && upp
						!= ("ELSIF") && upp != ("END") && upp != ("BETWEEN")
						&& upp != ("ASC") && upp != ("DESC") && upp != ("NULL"))
						&& !nokey) {

		} else if (upp == ("(")) {
			//             qDebug() << "start (";
			ret.subTokens().insert(ret.subTokens().end(), statement(
					statement::Token, token, tokens.line()));
			...
			if (com.startsWith(("--+")))
				com = ("/*+ ") + com.mid(3) + (" */");
			ret.subTokens().insert(ret.subTokens().end(), statement(
					statement::Token, com.simplified(), tokens.line()));
		} else if (upp.startsWith(("/*")) || upp.startsWith(("--"))
				|| upp.startsWith("//")) {
			//             qDebug() << "comment";
...
std::list<toSQLParse::statement> toSQLParse::parse(tokenizer &tokens) {
	...
	statement cur(statement::Statement);
	for (cur = parseStatement(tokens, false, false); cur.subTokens().begin()
			!= cur.subTokens().end(); cur
			= parseStatement(tokens, false, false)) {
		if (cur.Type == statement::List) {
			QMessageBox::warning(QApplication::activeWindow(), "Sqliteman",
					"toSQLparse: Unbalanced parenthesis (Too many ')')");
		}
	...
	if (!str.isEmpty())
		ret.insert(ret.end(), statement(statement::Raw, str, tokens.line()));
	return ret;
}
...
int toSQLParse::countIndent(const QString &txt, int &chars) {
	...
	while (txt[chars].isSpace() && chars < int(txt.length())) {
		...
		if (c == '\n')
			...
			level = (level / DEF_TAB_WIDTH
					/*SQLITEMAN toMarkedText::defaultTabWidth()*/+ 1)
					* DEF_TAB_WIDTH/*toMarkedText::defaultTabWidth()*/;
		chars++;
	}
	return level;
}

toSQLParse::settings toSQLParse::Settings = { true, false, false, false, true,
		true, true, 4, 60 };
...
static QString IndentComment(int level, int current, const QString &comment,
		bool endNl) {
	bool nl = true;
	QString ret;
	if (comment.length()) {
...
QString toSQLParse::indentStatement(statement &stat, int level/*SQLITEMAN, toSyntaxAnalyzer &syntax*/) {
	...
	case statement::Block: {
		...
		int exc = 0;
		for (std::list<toSQLParse::statement>::iterator i =
				stat.subTokens().begin(); i != stat.subTokens().end(); i++) {
			int add = 0;
			std::list<toSQLParse::statement>::iterator j = i;
			j++;
			if (i != stat.subTokens().begin() && j != stat.subTokens().end())
			...
			if (t == ("BEGIN") || t == ("WHEN") || t == ("ELSE") || t
					== ("ELSIF"))
				add = 0;
			if ((*i).Type == statement::List)
				ret += indentString(level + add + exc);
			ret += indentStatement(*i, level + add + exc/*SQLITEMAN , syntax*/);
			if ((*i).Type == statement::List) {
				int i;
	...
	case statement::Statement:
		...
		if (stat.Type == statement::Statement) {
			...
			for (std::list<toSQLParse::statement>::iterator i =
					stat.subTokens().begin(); i != stat.subTokens().end();) {
		...
		if (useMaxLev) {
			...
			for (std::list<toSQLParse::statement>::iterator i =
				...
				if (any) {
					...
					if ((*i).Type == statement::Keyword && upp != ("LOOP")
							&& upp != ("DO") && upp != ("THEN") && upp
							!= ("AS") && upp != ("IS")) {
		...
		for (std::list<toSQLParse::statement>::iterator i =
			...
			if ((*i).Type == statement::List) {
				...
				if (Settings.CommaBefore) {
					ret += IndentComment(Settings.CommentColumn, current,
							comment, true);
					comment = QString::null;
					ret += indentString(level + maxlev
							- (Settings.OperatorSpace ? 2 : 1));
					ret += (",");
				} else {
					ret += (",");
					ret += IndentComment(Settings.CommentColumn, current + 1,
							comment, true);
					comment = QString::null;
					ret += indentString(level + maxlev);
				}
				...
				lineList = true;
					...
					|| upp == ("DO") || upp == ("THEN") || upp == ("AS") || upp
					== ("IS"))) {
				if (!Settings.BlockOpenLine) {
					if (ret.length() > 0) {
						if (toIsIdent(ret.at(ret.length() - 1)) || ret.at(
								ret.length() - 1) == QUOTE_CHARACTER
								/*SQLITEMAN syntax.quoteCharacter()*/|| ret.at(
										ret.length() - 1) == '\''
					...
					ret += Settings.KeywordUpper ? (*i).String.toUpper()
							: (*i).String;
				...
				maxlev = maxlevorig;
				QString word = Settings.KeywordUpper ? (*i).String.toUpper()
						: (*i).String;
				if (ret.length()) {
				...
				bool add = false;
				if ((*i).Type == statement::Keyword) {
					if (!lineList && !any && (*i).Type == statement::Keyword
							&& !noKeyBreak && upp == ("BY"))
						add = true;
				} else {
					any = true;
				...
				if (isKeyword(upp)
						/*SQLITEMAN syntax.reservedWord(upp)*/&& Settings.KeywordUpper)
					t = upp;
				...
				if (first) {
					...
					if (ret.length() > 0 && !ret.at(ret.length() - 1).isSpace()
							&& (Settings.OperatorSpace
									|| ((toIsIdent(t[0]) || t[0]
											== QUOTE_CHARACTER
											/*SQLITEMAN syntax.quoteCharacter()*/|| t[0]
													== '\'')
											&& (toIsIdent(ret.at(ret.length()
													- 1)) || ret.at(
													ret.length() - 1)
													== QUOTE_CHARACTER
													/*SQLITEMAN syntax.quoteCharacter()*/|| ret.at(
															...
															== '\'')))) {
						if (t != (";") && t != (".")
								&& ret.at(ret.length() - 1) != '.' && current
								!= 0) {
							current++;
							ret += (" ");
						}
					} else if (ret.length() > 2 && ret.at(ret.length() - 2)
							== '*' && ret.at(ret.length() - 1) == '/') {
						current++;
						ret += (" ");
					}
		...
		if (stat.Type == statement::Statement) {
			ret
					+= IndentComment(Settings.CommentColumn, current, comment,
							true);
			comment = QString::null;
			if (Settings.EndBlockNewline && level == 0
...
QString toSQLParse::indent(const QString &str/*SQLITEMAN, toSyntaxAnalyzer &syntax*/) {
	...
	QString ret;
	for (std::list<toSQLParse::statement>::iterator i = blk.begin(); i
			!= blk.end(); i++) {
...
QString toSQLParse::indent(std::list<statement> &stat/*SQLITEMAN, toSyntaxAnalyzer &syntax*/) {
	...
	QString ret;
	for (std::list<toSQLParse::statement>::iterator i = stat.begin(); i
			!= stat.end(); i++) {
